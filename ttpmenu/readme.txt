...
TeraTerm Menu 1.23 (YYYY.MM.DD)
・初期設定時のデフォルトのフォント名が文字化けする不具合を修正した。 1.18 からの不具合。 (issue #1274)
・タスクトレイのアイコンを右クリック > 設定 に下記のオプションを追加した。 (issue #1293)
  セッションロック時にLockBoxパスワードをメモリから消去する。
  S4-Hibernateおよび従来のS3-Sleepへの移行時にLockBoxパスワードをメモリから消去する。
・メモリ内のパスワードを CryptProtectMemory() で暗号化するよう修正した。 (issue #1293)
・パスワード情報がページアウトされないよう修正した。 (issue #1293)
・メモリ使用量を削減した。 (issue #1293)
・テキストボックスに入力文字数制限を追加した。 (issue #1293)
・SecureZeroMemory()、境界外アクセスのチェック、エラー処理等を追加した。 (issue #1293)
・細かい調整を行った。 (issue #1234, #1285)
